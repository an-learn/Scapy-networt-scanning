...
def ips1_c(ic):
	answer = sr1(IP(dst = ips_list_c[ic])/ICMP(),timeout=1,verbose=0)
	if answer == None:
		pass
	else:
		print(ips_list_c[ic]+"\n")
...
def iptxt_start():
	filename = raw_input("[ping] Filename > ")
	if filename == "bk":
		print"===========================\n"
		select()
	elif filename == "exit":
...
def iptxt(ipt):
	answer = sr1(IP(dst=ipt.strip())/ICMP(),timeout=1,verbose=0)
	if answer == None:
		pass
	else:
		print(ipt.strip()+"\n")
