...
def select():
	answer3 = raw_input("[discovery-SYN] Select > ")
	if answer3 == "1.1":
		list_ips()
		ips1()
	elif answer3 == "1.2":
...
def ips1():
	...
	ips_list_h = []			# "-" hyphen
	ips_list_c = []			# "," comma
	ips_input = raw_input("[discovery-SYN] IP > ")
	if ips_input == "bk":
		print"===========================\n"
		select()
	elif ips_input == "exit":
...
def ips1_c(ic):
	answer1 = sr1(IP(dst = ips_list_c[ic])/TCP(dport=80,flags="S"),timeout=1,verbose=0)
	answer2 = sr1(IP(dst = ips_list_c[ic])/TCP(dport=445,flags="S"),timeout=1,verbose=0)
	answer3 = sr1(IP(dst = ips_list_c[ic])/TCP(dport=24242,flags="S"),timeout=1,verbose=0)
...
def ips2_h(addr):
	answer1 = sr1(IP(dst = prefix + str(addr))/TCP(dport=80,flags="S"),timeout=1,verbose=0)
	answer2 = sr1(IP(dst = prefix + str(addr))/TCP(dport=445,flags="S"),timeout=1,verbose=0)
	answer3 = sr1(IP(dst = prefix + str(addr))/TCP(dport=24242,flags="S"),timeout=1,verbose=0)
	if answer1 == None and answer2 == None and answer3 == None:
		pass
	else:
...
def iptxt_start():
	filename = raw_input("[discovery-SYN] Filename > ")
	if filename == "bk":
		print"===========================\n"
		select()
	elif filename == "exit":
...
def iptxt(ipt):
	answer1 = sr1(IP(dst=ipt.strip())/TCP(dport=80,flags="S"),timeout=1,verbose=0)
	answer2 = sr1(IP(dst=ipt.strip())/TCP(dport=445,flags="S"),timeout=1,verbose=0)
	answer3 = sr1(IP(dst=ipt.strip())/TCP(dport=24242,flags="S"),timeout=1,verbose=0)
	if answer1 == None and answer2 == None and answer3 == None:
		pass
	else:
		print(ipt.strip()+"\n")
