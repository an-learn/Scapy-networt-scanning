...
def ips():
	...
	ips_list_h = []			# "-" hyphen
	ips_list_c = []			# "," comma
	ips_input = raw_input("[scanning-Window] IP > ")
	if ips_input == "bk":
		print"===========================\n"
		main.select_port()
	elif ips_input == "exit":
...
def ports():
	...
	ports_list_h = []			# "-" hyphen
	ports_list_c = []			# "," comma
	ports_input = raw_input("[scanning-Window] Port > ")
	if ports_input == "bk":
		print"===========================\n"
		main.select_port()
...
def ports_scan_start():
	global ip,result,ports_list_cc,result_show
	ports_list_cc = ports_list_c
	result_show = []
	t1 = datetime.now()
	ports_c_num = len(ports_list_cc)
	print"===========================\n"
	print"All the result depend on the IP is alive!"
	for ip in ips_list_c:
		print"---------------------------\n"
		print("IP:\n ["+ip+"]")
		print" Scanning sevseal times to get accurater result"
		for s in range(1,1000):
			...
			q.join()
			if not sorted(ports_list_cc) == sorted(result):
				ports_list_cc = sorted(result)
				print(" "+str(s)+" times finished,"+'%.2f%%'%((len(ports_list_c)-len(result))/len(ports_list_c)*100)+" ascertained")
			else:
				print(" "+str(s)+" times finished,the result has no different than last time")
				print"The result is:"
				break
		print("IP:\n ["+ip+"]")
		print("Open Port("+str(len(result_show))+"/"+str(len(ports_list_c))+"):")
		print (" "+str(sorted(result_show)))
...
def ports_scan(port):
	try:
		answer = sr1(IP(dst=str(ip))/TCP(dport=ports_list_cc[port],flags="A"),timeout=1,verbose=0)
		if answer == None:
			result.append(ports_list_cc[port])
		else:
			if int(answer[TCP].window) > 0:
				result_show.append(ports_list_cc[port])
	except IndexError:
		pass
...
def tcpwindow_port_start():
	list_ips()
	ips()

if __name__ == '__main__':
	tcpwindow_port_start()
