...
def ips():
	...
	ips_list_h = []			# "-" hyphen
	ips_list_c = []			# "," comma
	ips_input = raw_input("[scanning-Maimon] IP > ")
	if ips_input == "bk":
		print"===========================\n"
		main.select_port()
	elif ips_input == "exit":
...
def ports():
	...
	ports_list_h = []			# "-" hyphen
	ports_list_c = []			# "," comma
	ports_input = raw_input("[scanning-Maimon] Port > ")
	if ports_input == "bk":
		print"===========================\n"
		main.select_port()
	...
	for ip in ips_list_c:
		...
		for s in range(1,1000):
			...
			if not sorted(ports_list_cc) == sorted(result):
				...
				break
		print("IP:\n ["+ip+"]")
		print("Open or Filtered Port("+str(len(result))+"/"+str(len(ports_list_c))+"):")
...
def ports_scan(port):
	try:
		answer = sr1(IP(dst=str(ip))/TCP(dport=ports_list_cc[port],flags="FA"),timeout=1,verbose=0)
		if answer == None:
...
def tcpmaimon_port_start():
	list_ips()
	ips()

if __name__ == '__main__':
	tcpmaimon_port_start()
