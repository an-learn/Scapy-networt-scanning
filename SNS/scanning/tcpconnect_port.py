...
def ips():
	...
	ips_list_h = []			# "-" hyphen
	ips_list_c = []			# "," comma
	ips_input = raw_input("[scanning-Connect] IP > ")
	if ips_input == "bk":
		print"===========================\n"
		main.select_port()
	elif ips_input == "exit":
...
def ports():
	...
	ports_list_h = []			# "-" hyphen
	ports_list_c = []			# "," comma
	ports_input = raw_input("[scanning-Connect] Port > ")
	if ports_input == "bk":
		print"===========================\n"
		main.select_port()
...
def ports_scan_start():
	...
	for ip in ips_list_c:
		asdasd = subprocess.call("iptables -A OUTPUT -p tcp --tcp-flags RST RST -d "+ip+" -j DROP",shell=True)
		result = []
		print"---------------------------\n"
		print("IP:\n ["+ip+"]")
		...
		q.join()
		asdasd = subprocess.call("iptables --flush",shell=True)
		print("Open Port("+str(len(result))+"/"+str(len(ports_list_c))+"):")
		print (" "+str(sorted(result)))
	print"\n\nfinish in "+str(datetime.now()-t1)
	print"---------------------------"
#	print"*Several iptables rule has been created."
#	print"*Suggest checking and flushing iptables."
#	print"---------------------------"
	ips()

def ports_scan(port):
	answer1 = sr1(IP(dst=str(ip))/TCP(dport=ports_list_c[port],flags="S",window=29200),timeout=1,verbose=0)
	if not str(type(answer1)) == "<type 'NoneType'>":
		if int(answer1[TCP].flags) == 18:
			answer2 = sr1(IP(dst=str(ip))/TCP(dport=ports_list_c[port],flags="A",ack=(answer1[TCP].seq+1),seq=(answer1[TCP].ack),window=29312),timeout=1,verbose=0)
			send(IP(dst=str(ip))/TCP(dport=ports_list_c[port],flags="RA",ack=(answer1[TCP].seq+1),seq=(answer1[TCP].ack),window=29312),verbose=0)
			result.append(ports_list_c[port])
...
def tcpconnect_port_start():
	list_ips()
	ips()

if __name__ == '__main__':
	tcpconnect_port_start()
