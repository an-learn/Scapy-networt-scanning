...
def list_select():
	print"===========================\n"
	print"1.Zombie IP test\n"
	print"2.Zombie scanning\n"

def list_zombieip():
	print"===========================\n"
	print"Enter zombie IP address to test usable or not\n"

def list_ips1():
	print"===========================\n"
	print"Enter one zombie IP address\n"

def list_ips2():
	print"===========================\n"
	print"Separate target IP address by \",\" (support \"1-254\")\n"

def list_ports():
	print"---------------------------\n"
	print"Separate Ports by \",\" (support \"1-65535\")"
	print"Or push \"Enter\" to use 1000 commonly used ports\n"
...
def zombieips():
	global zombieips_list_h,zombieips_list_c
	zombieips_list_h = []			# "-" hyphen
	zombieips_list_c = []			# "," comma
	zombieips_input = raw_input("[scanning-Zombie] zombieIP > ")
	if zombieips_input == "bk":
		print"===========================\n"
		select()
	elif zombieips_input == "exit":
		sys.exit()
	else:
		zombieips_list = zombieips_input.split(',')
		zombieips_list_unre = list(set(zombieips_list))		#qu chong fu
		for i in zombieips_list_unre:
...
def zombieports():
	global zombieports_list_h,zombieports_list_c
	zombieports_list_h = []			# "-" hyphen
	zombieports_list_c = []			# "," comma
	zombieports_input = raw_input("[scanning-Zombie] zombiePort > ")
	if zombieports_input == "bk":
		print"===========================\n"
		select()
	elif zombieports_input == "exit":
		sys.exit()
	elif zombieports_input == "":
		zombieports_list_c = [22,55,80,445,2992,22322]
		ports_scan_start()
	else:
		zombieports_list = zombieports_input.split(',')
		zombieports_list_unre = list(set(zombieports_list))		#qu chong fu
		for i in zombieports_list_unre:
			if re.match(r"^(?:(?:[0-9]\-)|(?:[1-9][0-9]\-)|(?:[1-9][0-9][0-9]\-)|(?:[1-9][0-9][0-9][0-9]\-)|"
			+"(?:[1-5][0-9][0-9][0-9][0-9]\-)|(?:6[0-4]0-9][0-9][0-9]\-)|(?:65[0-4][0-9][0-9]\-)|(?:655[0-2][0-9]\-)|(?:6553[0-5]\-))"
			+"(?:(?:[0-9])|(?:[1-9][0-9])|(?:[1-9][0-9][0-9])|(?:[1-9][0-9][0-9][0-9])|"
			+"(?:[1-5][0-9][0-9][0-9][0-9])|(?:6[0-4]0-9][0-9][0-9])|(?:65[0-4][0-9][0-9])|(?:655[0-2][0-9])|(?:6553[0-5]))$",i):
				zombieports_list_h.append(i)
			elif re.match(r"^(?:(?:[0-9])|(?:[1-9][0-9])|(?:[1-9][0-9][0-9])|(?:[1-9][0-9][0-9][0-9])|"
			+"(?:[1-5][0-9][0-9][0-9][0-9])|(?:6[0-4]0-9][0-9][0-9])|(?:65[0-4][0-9][0-9])|(?:655[0-2][0-9])|(?:6553[0-5]))$",i):
				zombieports_list_c.append(i)
			else:
				print"===========================\n"
...
def zombieips_ok():
	global zombieips_list_h,zombieips_list_c
	zombieips_h_num = len(zombieips_list_h)
	for ih in range(0,zombieips_h_num):
		addr1 = int(zombieips_list_h[ih].split('.')[3].split('-')[0])
		addr2 = int(zombieips_list_h[ih].split('.')[3].split('-')[1])
		prefix = zombieips_list_h[ih].split('.')[0] + '.' + zombieips_list_h[ih].split('.')[1] + '.' + zombieips_list_h[ih].split('.')[2] + '.'
		for addr in range(addr1,addr2+1):
			zombieiph = prefix + str(addr)
			zombieips_list_c.append(zombieiph)
	zombieips_list_c = list(set(zombieips_list_c))
	zombieports_ok()

def zombieports_ok():
	global zombieports_list_h,zombieports_list_c
	zombieports_h_num = len(zombieports_list_h)
	for ih in range(0,zombieports_h_num):
		port1 = int(zombieports_list_h[ih].split('-')[0])
		port2 = int(zombieports_list_h[ih].split('-')[1])
		for zombieport in range(port1,port2+1):
			zombieports_list_c.append(zombieport)
	zombieports_list_c = list(set(zombieports_list_c))
	zombieports_list_c = [int(i) for i in zombieports_list_c]
	zombie_test()
...
def zombie_test():
	...
	zombie_show = []
	t1 = datetime.now()
	for zombieip in zombieips_list_c:
		for zombieport in zombieports_list_c:
			answer1 = sr1(IP(dst=zombieip)/TCP(dport=int(zombieport),flags="SA"), timeout=1,verbose=0)
			if not str(type(answer1)) == "<type 'NoneType'>":
				send(IP(dst=zombieip)/TCP(dport=int(zombieport),flags="SA"),verbose=0)
				send(IP(dst=zombieip)/TCP(dport=int(zombieport),flags="SA"),verbose=0)
				answer2 = sr1(IP(dst=zombieip)/TCP(dport=int(zombieport),flags="SA"), timeout=1,verbose=0)
				if not str(type(answer2)) == "<type 'NoneType'>" and answer2[IP].id == (answer1[IP].id+3):
					zombie_show.append(zombieip+":"+str(zombieport))
	print"===========================\n"
	if len(zombie_show) == 0:
		print" No IP can be zombie IP"
	for i in zombie_show:
		print " ["+i+"] can be a zombie IP"
...
def ips():
	global ips_list_h,ips_list_c,zombieip,zombieport
	zombieip = raw_input("[scanning-Zombie] zombieIP > ")
	if zombieip == "bk":
		print"===========================\n"
		select()
	elif zombieip == "exit":
		sys.exit()
	elif not re.match(r"^(?:(?:1[0-9][0-9]\.)|(?:2[0-4][0-9]\.)|(?:25[0-5]\.)|(?:[1-9][0-9]\.)|(?:[0-9]\.)){3}(?:(?:1[0-9][0-9])|(?:2[0-4][0-9])|(?:25[0-5])|(?:[1-9][0-9])|(?:[0-9]))$",zombieip):
		print"===========================\n"
		print("Include incorrect IP format: "+zombieip)
		print"Try again\n"
		ips()
	print"---------------------------"
	zombieport = raw_input("[scanning-Zombie] zombiePort > ")
	if zombieport == "bk":
		print"===========================\n"
		select()
	elif zombieport == "exit":
		sys.exit()
	elif not re.match(r"^(?:(?:[0-9])|(?:[1-9][0-9])|(?:[1-9][0-9][0-9])|(?:[1-9][0-9][0-9][0-9])|"
			+"(?:[1-5][0-9][0-9][0-9][0-9])|(?:6[0-4]0-9][0-9][0-9])|(?:65[0-4][0-9][0-9])|(?:655[0-2][0-9])|(?:6553[0-5]))$",zombieport):
		print"===========================\n"
		print("Include incorrect Port format: "+zombieport)
		print"Try again\n"
		ips()
	ips_list_h = []			# "-" hyphen
	ips_list_c = []			# "," comma
	list_ips2()
	ips_input = raw_input("[scanning-Zombie] IP > ")
	if ips_input == "bk":
		print"===========================\n"
		select()
	elif ips_input == "exit":
...
def ports():
	...
	ports_list_h = []			# "-" hyphen
	ports_list_c = []			# "," comma
	ports_input = raw_input("[scanning-Zombie] Port > ")
	if ports_input == "bk":
		print"===========================\n"
		select()
...
def ports_ok():
	global ports_list_h,ports_list_c
	ports_h_num = len(ports_list_h)
	for ih in range(0,ports_h_num):
		port1 = int(ports_list_h[ih].split('-')[0])
		port2 = int(ports_list_h[ih].split('-')[1])
		for port in range(port1,port2+1):
			ports_list_c.append(port)
	ports_list_c = list(set(ports_list_c))
	ports_list_c = [int(i) for i in ports_list_c]
	zombie_scan()
...
def zombie_scan():
	...
	result = []
	print"===========================\n"
	print"All the result depend on the IP is alive!"
	for ip in ips_list_c:
		print"---------------------------\n"
		print("IP:\n ["+ip+"]")
		for port in ports_list_c:
			start_val = sr1(IP(dst=zombieip)/TCP(flags="SA",dport=int(zombieport)),timeout=1,verbose=0)
			if not str(type(start_val)) == "<type 'NoneType'>":
				send(IP(src=zombieip,dst=ip)/TCP(flags="S",sport=int(zombieport),dport=port),verbose=0)
				end_val = sr1(IP(dst=zombieip)/TCP(flags="SA",dport=int(zombieport)),timeout=1,verbose=0)
				if not str(type(end_val)) == "<type 'NoneType'>" and end_val[IP].id == (start_val[IP].id+2):
					result.append(port)
		print("Open Port("+str(len(result))+"/"+str(len(ports_list_c))+"):")
...
def tcpzombie_port_start():
	list_select()
	select()
	#zombieips()

if __name__ == '__main__':
	tcpzombie_port_start()
