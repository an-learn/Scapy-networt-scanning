...
def list_flags():
	print"---------------------------\n"
	print"Enter flags as \"sAruPF\"\n"

def ips():
	global ips_list_h,ips_list_c
	ips_list_h = []			# "-" hyphen
	ips_list_c = []			# "," comma
	ips_input = raw_input("[scanning-DIY] IP > ")
	if ips_input == "bk":
		print"===========================\n"
		main.select_port()
	elif ips_input == "exit":
...
def ports():
	...
	ports_list_h = []			# "-" hyphen
	ports_list_c = []			# "," comma
	ports_input = raw_input("[scanning-DIY] Port > ")
	if ports_input == "bk":
		print"===========================\n"
		main.select_port()
...
def ports_scan_start():
	...
	for ip in ips_list_c:
		...
		q.join()
		print("Response Port("+str(len(result))+"/"+str(len(ports_list_c))+"):")
...
def ports_scan(port):
	answer = sr1(IP(dst=str(ip))/TCP(dport=ports_list_c[port],flags=diyflags),timeout=1,verbose=0)
	if answer == None:
		pass
	else:
		result.append(" "+str(ports_list_c[port])+" responses flags = "+str(answer[TCP].flags))
