...
def ips():
	...
	ips_list_h = []			# "-" hyphen
	ips_list_c = []			# "," comma
	ips_input = raw_input("[scanning-Null] IP > ")
	if ips_input == "bk":
		print"===========================\n"
		main.select_port()
	elif ips_input == "exit":
...
def ports():
	...
	ports_list_h = []			# "-" hyphen
	ports_list_c = []			# "," comma
	ports_input = raw_input("[scanning-Null] Port > ")
	if ports_input == "bk":
		print"===========================\n"
		main.select_port()
...
def ports_scan(port):
	try:
		answer = sr1(IP(dst=str(ip))/TCP(dport=ports_list_cc[port],flags=""),timeout=1,verbose=0)
		if answer == None:
