...
def ips():
	...
	ips_list_h = []			# "-" hyphen
	ips_list_c = []			# "," comma
	ips_input = raw_input("[Banner] IP > ")
	if ips_input == "bk":
		print"===========================\n"
		main.select_fingerprinting()
	elif ips_input == "exit":
		sys.exit()
	else:
...
def ports():
	...
	ports_list_h = []			# "-" hyphen
	ports_list_c = []			# "," comma
	ports_input = raw_input("[Banner] Port > ")
	if ports_input == "bk":
		print"===========================\n"
		main.select_fingerprinting()
...
def ports_scan(port):
	...
	try:
		...
		banner.connect((ip,ports_list_c[port]))
		ready = select.select([banner],[],[],1)
		if ready[0]:
			print("Port "+str(ports_list_c[port])+": "+banner.recv(4096))
			banner.close()
		else:
			pass
	except:
