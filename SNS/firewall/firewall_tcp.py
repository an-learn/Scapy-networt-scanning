...
def ips():
	...
	ips_list_h = []			# "-" hyphen
	ips_list_c = []			# "," comma
	ips_input = raw_input("[Firewall] IP > ")
	if ips_input == "bk":
		print"===========================\n"
		main.select_firewall()
	elif ips_input == "exit":
		sys.exit()
	else:
...
def ports():
	...
	ports_list_h = []			# "-" hyphen
	ports_list_c = []			# "," comma
	ports_input = raw_input("[Firewall] Port > ")
	if ports_input == "bk":
		print"===========================\n"
		main.select_firewall()
...
def ports_scan_start():
	global ip,result_sn_an,result_sn_a,result_s_an,result_ssa_ar,result_ssr_ar,result_unkonwn
	t1 = datetime.now()
	ports_c_num = len(ports_list_c)
	print"===========================\n"
	...
	for ip in ips_list_c:
		...
		print"---------------------------\n"
		print("IP:\n ["+ip+"]")
		map(q.put,xrange(0,ports_c_num))
		threads = [threading.Thread(target=worker_scan) for i in xrange(50)]
		map(lambda x:x.start(),threads)
		q.join()
		print("Unfiltered  Open Port("+str(len(result_ssa_ar))+"/"+str(len(ports_list_c))+"):")
		print (" "+str(sorted(result_ssa_ar)))
		print("Unfiltered Close Port("+str(len(result_ssr_ar))+"/"+str(len(ports_list_c))+"):")
		print (" "+str(sorted(result_ssr_ar)))
		print("Filtered-in Port("+str(len(result_sn_a))+"/"+str(len(ports_list_c))+"):")
		print (" "+str(sorted(result_sn_a)))
		print("Filtered-out Port("+str(len(result_s_an))+"/"+str(len(ports_list_c))+"):")
		print (" "+str(sorted(result_s_an)))
		print("Unstatefully filtered("+str(len(result_sn_an))+"/"+str(len(ports_list_c))+"):")
		print (" "+str(sorted(result_sn_an)))
		print("Uknown Port("+str(len(result_unkonwn))+"/"+str(len(ports_list_c))+"):")
		print (" "+str(sorted(result_unkonwn)))
	print"\n\nfinish in "+str(datetime.now()-t1)
	print"---------------------------"
	ips()

def ports_scan(port):
	answer_s = sr1(IP(dst=str(ip))/TCP(dport=ports_list_c[port]),timeout=1,verbose=0)
	answer_a = sr1(IP(dst=str(ip))/TCP(dport=ports_list_c[port],flags="A"),timeout=1,verbose=0)
	if answer_s == None and answer_a == None:
		result_sn_an.append(ports_list_c[port])
	elif answer_s == None and not answer_a == None:
		result_sn_a.append(ports_list_c[port])
	elif not answer_s == None and answer_a == None:
		result_s_an.append(ports_list_c[port])
	elif int(answer_s[TCP].flags) == 18 and int(answer_a[TCP].flags) == 4:
		result_ssa_ar.append(ports_list_c[port])
	elif int(answer_s[TCP].flags) == 20 and int(answer_a[TCP].flags) == 4:
		result_ssr_ar.append(ports_list_c[port])
	else:
		result_unkonwn.append(ports_list_c[port])
